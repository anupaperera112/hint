...
static duckdb::unique_ptr<duckdb::FunctionData> HintSearchBind(
...
static duckdb::unique_ptr<duckdb::GlobalTableFunctionState> HintSearchInit(
...
static void HintSearchExecute(
    duckdb::ClientContext &context, duckdb::TableFunctionInput &data_input,
...
// ============================================================================
vector<RecordId> generateDeletes(size_t count, size_t maxId, unsigned seed = 123) {
    mt19937 rng(seed);
                                  ...
                                  const vector<RangeQuery>& queries)
{
    ...
    for (size_t i = 0; i < queries.size(); i++) {
        string sql = "SELECT count(*) FROM intervals WHERE start_ts <= " +
                     to_string(queries[i].end) + " AND end_ts >= " +
                     to_string(queries[i].start);
        auto result = con.Query(sql);
        if (!result->HasError()) {
    ...
    // Verification
    for (size_t i = 0; i < queries.size(); i++) {
        if (hint_results[i] != scan_results[i]) res.mismatches++;
    }
...
int main()
{
    ...
    cout << "╔══════════════════════════════════════════════════════════════════════╗" << endl;
    cout << "║     Dynamic HINT Index + DuckDB Benchmark                          ║" << endl;
    cout << "║     Threshold-Based Auto-Rebuild Performance Analysis              ║" << endl;
    cout << "╚══════════════════════════════════════════════════════════════════════╝" << endl;
    cout << endl;
    ...
    cout << "\n  Verification        : " << (baseline.mismatches == 0 ? "PASS" : "FAIL")
         << " (" << baseline.mismatches << " mismatches)" << endl;
    cout << "  ┌──────────────────────┬─────────────────┬─────────────────────┐" << endl;
    cout << "  │ Method               │ Latency (ms/op) │ Throughput (ops/s)  │" << endl;
    cout << "  ├──────────────────────┼─────────────────┼─────────────────────┤" << endl;
    ...
    cout << "  Inserting " << NUM_INSERTS << " records with threshold=100 ..." << flush;
    ...
    cout << "\n  Verification        : " << (postInsert.mismatches == 0 ? "PASS" : "FAIL")
    ...
    while (insIdx < insertRecords.size() || delIdx < deleteIds.size()) {
        ...
        for (int b = 0; b < 5 && insIdx < insertRecords.size(); b++, insIdx++) {
            hint_mixed.insert(insertRecords[insIdx].first, insertRecords[insIdx].second);
        }
        // Delete 1
    ...
    cout << "\n  Verification        : " << (postMixed.mismatches == 0 ? "PASS" : "FAIL")
    ...
    cout << "\n  ┌───────────┬──────────┬───────────┬─────────────┬──────────────────┬─────────┬────────┐" << endl;
    cout << "  │ Threshold │ Rebuilds │ Ops (s)   │ Query (ms)  │ Throughput (q/s) │ Speedup │ Verify │" << endl;
    cout << "  ├───────────┼──────────┼───────────┼─────────────┼──────────────────┼─────────┼────────┤" << endl;
    ...
    for (unsigned int thresh : thresholds) {
        ...
        printf("  │ %9u │ %8zu │ %9.4f │ %11.4f │ %16.0f │ %7.2f │ %s  │\n",
               thresh,
               hint_sweep.numMerges,
               sw_ops_time,
               sw_result.hint_latency_ms,
               sw_result.hint_throughput,
               sw_result.speedup,
    ...
    cout << "  Measuring raw HINT^m query performance (bypassing DuckDB) ..." << endl;
    ...
    cout << "  ┌──────────────────────────┬──────────────┬──────────────────┬─────────┬────────┐" << endl;
    cout << "  │ Scenario                 │ Latency (ms) │ Throughput (q/s) │ Speedup │ Verify │" << endl;
    cout << "  ├──────────────────────────┼──────────────┼──────────────────┼─────────┼────────┤" << endl;

    auto printSummaryRow = [](const char* label, const BenchmarkResult& r) {
        printf("  │ %-24s │ %12.4f │ %16.0f │ %7.2f │ %s  │\n",
               label, r.hint_latency_ms, r.hint_throughput, r.speedup,
               r.mismatches == 0 ? "PASS" : "FAIL");
    };
    ...
    printf("  │ %-24s │ %12.4f │ %16.0f │     ref │  N/A  │\n",
           "DuckDB Full Scan (ref)", baseline.scan_latency_ms, baseline.scan_throughput);
    cout << "  └──────────────────────────┴──────────────┴──────────────────┴─────────┴────────┘" << endl;
