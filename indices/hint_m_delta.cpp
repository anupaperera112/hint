...
HINT_M_Dynamic::~HINT_M_Dynamic() { delete this->mainIndex; }
...
void HINT_M_Dynamic::merge() {
  ...
  for (const Record &r : this->baseRelation) {
    if (this->deltaDeletes.find(r.id) == this->deltaDeletes.end()) {
      ...
      newRelation.longestRecord =
          std::max(newRelation.longestRecord, (Timestamp)(r.end - r.start + 1));
      sum += r.end - r.start;
    }
  }
  ...
  unsigned int newMaxBits =
      (newRelation.gend > newRelation.gstart)
          ? (unsigned int)(log2(newRelation.gend - newRelation.gstart) + 1)
          : 1;

  // Step 3: Recalculate numBits via cost model if auto, else keep
  // user-specified
...
void HINT_M_Dynamic::forceRebuild() { this->merge(); }
